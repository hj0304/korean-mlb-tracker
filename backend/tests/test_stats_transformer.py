...
def test_transform_year_by_year_keeps_each_level() -> None:
    """A player at MLB + AAA in the same season keeps a row per level, with the
    most-games split chosen within each level."""
    ...
    by_key = {(r["season"], r["level"]): r["stats"] for r in rows}
    # 2025 played both levels -> both rows survive (previously only AAA did)
    assert by_key[(2025, "MLB")]["avg"] == ".050"
    assert by_key[(2025, "AAA")]["avg"] == ".292"
    # 2023 too: MLB (111 G) and AAA (9 G) both kept
    assert by_key[(2023, "MLB")]["avg"] == ".231"
    assert by_key[(2023, "AAA")]["avg"] == ".344"
    # 2026 was AAA only
    assert (2026, "AAA") in by_key
    assert (2026, "MLB") not in by_key
...
def test_transform_game_log_snapshot() -> None:
    ...
    result = stats_transformer.transform_game_log(
        box, player_id=808975, game_id=822832, game_date="2026-04-06", level="MLB"
    )
...
def test_transform_game_log_player_absent() -> None:
    ...
    assert (
        stats_transformer.transform_game_log(
            box, player_id=1, game_id=822832, game_date="2026-04-06", level="MLB"
        )
        == []
    )
