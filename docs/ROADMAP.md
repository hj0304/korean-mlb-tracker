...
- [x] **S2-07** `RecentGamesChart.tsx`: Recharts로 경기별 추이 차트 — 타자=경기 타율(시즌 타율 기준선), 투수=경기 ERA(시즌 ERA 기준선). 단일 경기 지표라 OPS 대신 타율 사용(타석당 OPS는 잡음 큼). 투수 ERA는 짧은 등판이 20~30대로 튀어 정상 경기를 납작하게 만들므로 플롯값을 13.5로 캡(툴팁은 실제값), 레벨 탭과 연동
...
#### 품질
- [x] **S2-12** Lighthouse 측정 → 저위험 폴리시 적용. PageSpeed Insights(홈) **데스크톱 97 / 모바일 87** — A11y/BP/SEO 100, 폰트는 next/font가 이미 self-host+preload. 적용: 홈 CLS 0.20→0.09(로딩 스켈레톤을 실제 레이아웃에 맞춤), recharts dynamic import로 초기 번들에서 344K 분리. 데스크톱은 90+ 달성, 모바일은 87(근접). 남은 모바일 격차의 근본은 클라이언트 fetch 구조(상세 LCP) → SSR prefetch는 후속(Sprint 5+ 폴리시 백로그)
...
- [x] **S2-17** "명예 한국인" 선수 추가 — 한국 출생은 아니나 부모가 한국 국적이라 대표팀으로 뛴 선수(에드먼/더닝/위트컴/존스/오브라이언). `players.is_honorary` 플래그 + `HONORARY_PLAYERS` 시드(#76 백엔드). 목록에 "전체/한국인/명예 한국인" 필터(레벨과 별개 축) + 카드·상세 배지(#77 프론트)
...
## Sprint 4 — "태극기 펄럭이며": 리브랜딩 + 로그인 + 즐겨찾기 (목표: 5~7일)

**Definition of Done**: 비로그인 방문자는 새 랜딩 페이지를 보고, 로그인(Google/카카오/이메일)하면 트래커로 진입해 선수를 즐겨찾기에 담고 즐겨찾기 필터로 볼 수 있다. 서비스명이 '태극기 펄럭이며'로 바뀌어 있다.

스코프 변경 근거: PRD §3 비목표(회원가입/로그인) v1.1 해제 + `docs/adr/0009-supabase-auth-favorites.md`

### 작업

- [x] **S4-01** 문서 + 리브랜딩 — PRD 비목표 해제·P1.5 기능 추가, ADR-0009, 서비스명 '태극기 펄럭이며' 적용(frontend metadata/OG 이미지/README), CLAUDE.md 스코프 갱신
- [x] **S4-02** 디자인 리뉴얼 — 랜딩 페이지(네이비 + 태극 실루엣 히어로, 정적, `/`) + 트래커는 `/dashboard`로 이동 + 메인 리스타일(상단 내비게이션 바 `SiteHeader`, 웜톤 배경 + 네이비 primary 테마 토큰, 탭 → 알약(pill) 스타일, 피드 카드화, 하이라이트 배너 레드 틴트). 로그인 분기·로그인 버튼은 S4-03에서
- [x] **S4-06** 디자인 v2 — 사용자 와이어프레임 반영: 랜딩을 다크(#07090c)+레드(#e23b2e) 스포티 스타일로 재구현(스타디움 라이트 그라디언트, 마스크 처리한 선수 사진 히어로, Black Han Sans 헤드라인, 최근 경기 마퀴 티커), 앱 전체 토큰을 다크 기본+레드 액센트로 교체, 폰트 스택 Noto Sans KR/Black Han Sans/Bebas Neue/JetBrains Mono. 후속: 헤더 로고를 실시간 펄럭이는 태극기(SVG feTurbulence/feDisplacementMap + SMIL)로 교체
- [ ] **S4-03** 로그인 (Supabase Auth) — Google/카카오 OAuth + 이메일·비밀번호, 로그인 모달, 로그인 상태 분기(랜딩은 비로그인 시에만, 헤더 프로필 칩), 백엔드 JWT 검증 의존성. **선행 수동 작업**: Google Cloud Console·Kakao Developers OAuth 앱 등록 → Supabase 대시보드 연결
- [ ] **S4-04** 즐겨찾기 백엔드 — `favorites(user_id, player_id)` 마이그레이션, 등록/해제/목록 API(JWT 필수), 대시보드 피드 즐겨찾기 필터 파라미터
- [ ] **S4-05** 즐겨찾기 프론트엔드 — 선수 카드/행 별 버튼(등록·해제), 오늘의 경기 `전체|즐겨찾기` 토글, 선수 목록 `즐겨찾기` 카테고리 탭

**산출물**: 로그인하고 내 선수만 골라 보는 개인화된 사이트. 랜딩 페이지로 첫인상 개선.

---

## Sprint 5+ — P1 이후 (출시 후, 시즌 종료까지)
...
| Sprint 2 | | | | |
| Sprint 3 | | | | |
| Sprint 4 | 2026-07-07 | | | 리브랜딩 + 로그인 + 즐겨찾기 |
